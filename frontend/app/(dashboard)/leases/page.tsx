...
export default function LeasesPage() {
    ...
    const handleRowClick = (tenant: any) => {
        console.log("Lease clicked:", tenant);
    ...
    const leasesColumns: ColumnDef<LeaseTableData>[] = [
        ...
        {
            accessorKey: 'agreementStartEndDate',
            header: 'Period',
            cell: ({ row }) => {
                const start = row.original.agreementStartDate ? new Date(row.original.agreementStartDate).toLocaleDateString('en-GB') : '-';
                const end = row.original.agreementEndDate ? new Date(row.original.agreementEndDate).toLocaleDateString('en-GB') : '-';
                return `${start} - ${end}`;
            },
            size: 120,
        },
        {
            accessorKey: 'leaseAmountPaid',
            header: 'Amount Paid',
            cell: ({ row }) => {
                if (!row.original.invoices || row.original.invoices.length === 0) return '-';
                const totalPaid = row.original.invoices.reduce((sum, invoice) => sum + parseFloat(invoice.paidAmount || 0), 0);
                return `KES ${totalPaid.toLocaleString()}`;
            },
            size: 120,
        },
        {
            accessorKey: 'leaseAmountBalance',
            header: 'Balance',
            cell: ({ row }) => {
                if (!row.original.invoices || row.original.invoices.length === 0) return '-';
                const totalBalance = row.original.invoices.reduce((sum, invoice) => sum + parseFloat(invoice.balanceAmount || 0), 0);
        ...
        <div className="space-y-6">
            ...
            <ExpandableTable
                data={tenants.map(tenant => ({
                    ...
                    invoices: tenant.rentalAgreement?.invoices || [],
                    rentStatus: '',
